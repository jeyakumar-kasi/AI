...
for root_dir, dirs, files in os.walk(img_dir):
    for file in files:
        if os.path.splitext(file)[1] in filter_img_ext:
            ...
            # Detect the faces
            faces = faceCascade.detectMultiScale(img_arr, scaleFactor=1.1, minNeighbors=5)
            if len(faces) != 1:
                # If no faces or two or more faces detected (i.e Group Photo) then skip it.
                if len(faces) == 0:
                    print(f'WARN: <{person_name}: {file}> No face is detected.')
                else:
                    print(f'WARN: <{person_name}: {file}> {len(faces)} Face(s) are detected. Considering as a Group photo. Skipped.')
                #! os.remove(filepath)
                continue
